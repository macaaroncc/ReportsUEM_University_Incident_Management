...
public class _08_CrearIncidencia extends JFrame {
	...
	public _08_CrearIncidencia() {
		...
		btnSeleccionarFoto.addActionListener(e -> {
			...
			if (result == JFileChooser.APPROVE_OPTION) {
				...
				try {
					...
					if (imagen != null) {
						Image imagenEscalada = imagen.getScaledInstance(lblPreview.getWidth(), lblPreview.getHeight(),
								Image.SCALE_SMOOTH);
		...
		btnCrear.addActionListener(e -> {
			if (controlador != null) {
				controlador.crearIncidencia(comboEdificio.getSelectedItem().toString(), imagenBytes,
						comboPiso.getSelectedItem().toString(), txtDescripcion.getText(), txtAula.getText(),
						comboCampus.getSelectedItem().toString(), this
