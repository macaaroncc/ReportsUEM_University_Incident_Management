...
public class Controlador {
    ...
    /**
     * Obtiene las preguntas de seguridad del usuario y actualiza la UI para mostrarlas.
     */
    ...
    /**
     * Verifica las respuestas dadas para las preguntas de seguridad.
     */
    public void comprobarPreguntasSeguridad(String email, String respuesta1, String respuesta2, JFrame ventanaActual) {
        try (Connection conn = ConexionBD.conectar();
             PreparedStatement stmt = conn.prepareStatement("SELECT RESP1, RESP2 FROM SEGURIDAD WHERE USERS_USR = ?")) {

            stmt.setString(1, email);
            ResultSet rs = stmt.executeQuery();

            if (rs.next()) {
                String r1 = rs.getString("RESP1").trim();
                String r2 = rs.getString("RESP2").trim();

                if (r1.equalsIgnoreCase(respuesta1) && r2.equalsIgnoreCase(respuesta2)) {
                    _05_RestContrasena rest = new _05_RestContrasena();
                    rest.setControlador(this);
            ...
            JOptionPane.showMessageDialog(null, "Error al verificar respuestas:\n" + e.getMessage());
            e.printStackTrace();
        }
    }
